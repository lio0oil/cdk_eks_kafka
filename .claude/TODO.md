...
- **alertmanager の採用是非を決定する**
  - 現状: [manifests/monitoring/kube-prometheus-stack-values.yaml:48-49](manifests/monitoring/kube-prometheus-stack-values.yaml#L48-L49) で `enabled: false`（通知先・alert rule が未整備のため明示的にオフ）。採用する場合は (1) Prometheus alert rule（`KafkaBrokerDown` 等）の定義、(2) `alertmanager.config` の receiver（Slack / PagerDuty 等）と routing、(3) webhook URL を Secret で参照する仕組み、の 3 点をセットで決める必要がある。
