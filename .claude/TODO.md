...
- **broker/controller の `resources.requests/limits`・`storage.size`・`jvmOptions` を stg/prd 向けにチューニングする**
  ...
  - チューニングの判断材料を得るための性能測定アーキテクチャ調査は [PERFTEST.md](PERFTEST.md) を参照（未実装、Locust + confluent-kafka を推奨）。
- **Prometheus / Alertmanager / Fluent Bit の性能パラメータ（`replicas` / `retention` / `storageSpec` / `scrapeInterval` / `resources` / バッファサイズ）を stg/prd 向けにチューニングする**
  - 現状: `ClusterConfig.prometheus_resources`（`PrometheusResourceConfig`: memory/cpu/storage/retention/scrapeInterval）/ `alertmanager_resources`（`AlertmanagerResourceConfig`: memory/cpu/storage）/ `fluent_bit_resources`（`FluentBitResourceConfig`: memory/cpu/mem_buf_limit）/ `prometheus_replicas` / `alertmanager_replicas`（[config.py](../cdk/ekscdk/config.py)、注入先は [manifests/monitoring/kube-prometheus-stack-values.yaml](../cdk/manifests/monitoring/kube-prometheus-stack-values.yaml) / [manifests/monitoring/fluent-bit-values.yaml](../cdk/manifests/monitoring/fluent-bit-values.yaml)）で環境別に上書きできる仕組みは実装済みだが、dev/stg/prd とも同じ暫定値（Prometheus replicas 2・retention 15d・storage 20Gi・scrapeInterval 30s・resources memory 512Mi-1Gi/cpu 250-500m、Alertmanager replicas 3・storage 1Gi・resources memory 64Mi-128Mi/cpu 50-100m、Fluent Bit resources memory 128Mi-256Mi/cpu 50-200m・Mem_Buf_Limit 5MB）のまま。SLO 月次レポート要件が出れば retention を伸ばし、それに連動して storage size も引き上げる。`broker_count` 増設で per-broker metrics（kafka-exporter/JMX）・通知量・ログ流量が増えた場合は resources / バッファサイズも見直す。
  - 対象外: PrometheusRule のしきい値（アラート発火条件、[prometheus-rules-kafka.yaml](../cdk/manifests/monitoring/prometheus-rules-kafka.yaml) 等）は resources と性質が異なる（検知基準であり性能パラメータではない）ため、環境間で同値のままでよいと判断し config 化していない。
