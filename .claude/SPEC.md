...
| 8 | `prometheus-install/prometheus-rules/*`（8個） | 一部 | kafka-rules / kafka-exporter-topic / cluster-operator-rules / entity-operator-rules / certificate-rules の5個を`manifests/monitoring/prometheus-install/prometheus-rules/`に配置。bridge / connect / mirrormaker2 は未使用コンポーネントのため対象外。kafka-rulesのみ、KafkaNodePool名がbroker/controller（公式サンプル想定のkafkaに加えて実際に存在する名前）に対応するためPod/PVC名regexを変更している（他4件は無編集）。ScrapeProblem/KafkaContainerRestartedInTheLast5Minutes/KafkaRunningOutOfSpaceは`.+-(kafka\|broker\|controller)-[0-9]+`（broker/controllerどちらの障害でも意味が壊れないため両方含める）、KafkaBrokerContainersDownのみ`.+-(kafka\|broker)-[0-9]+`（absent()ベースのため両方含めると「両方同時に全滅」しないと発火しなくなり検知力が落ちる） |
...
| 16 | `kafka-connect-metrics.yaml` | 対象外 | Connect未使用 |
| 17 | `kafka-cruise-control-metrics.yaml` | 採用 | ConfigMap部分（`cruise-control-metrics` / key `metrics-config.yml`）を`manifests/monitoring/kafka-cruise-control-metrics.yaml`に無編集で配置 |
| 18 | `kafka-metrics.yaml` | 採用 | ConfigMap部分（`kafka-metrics` / key `kafka-metrics-config.yml`）を`manifests/monitoring/kafka-metrics.yaml`に無編集で配置。Kafka broker用とCruise Control用は公式同様に別ConfigMapとして扱い、1つに統合しない |
| 19 | `kafka-mirror-maker-2-metrics.yaml` | 対象外 | MirrorMaker2未使用 |
| 20 | `oauth-metrics.yaml` | 対象外 | OAuth未使用 |
...
### system-nodegroup は kafka_single_az と同じ AZ ポリシーに従う
`system-nodegroup` は `EksClusterConstruct` 内で kafka-broker/controller と共用する `nodegroup_subnets` を使うため、`kafka_single_az` の値で挙動が変わる。

- **`kafka_single_az=True`（dev）**: system も 1 AZ 目に固定する。AZ 障害時の自動フェイルオーバーを用意していないため、監視（Prometheus/Alertmanager 等）だけ multi-AZ に残しても得られる価値は「通知が届くタイミングが早まる」程度で対応不能な点は変わらず、AZ 間 scrape トラフィック分のコストに見合わない。
- **`kafka_single_az=False`（stg/prd）**: `NetworkConstruct` の VPC は `max_azs=3` で 3 AZ 構成のため、`system_desired_size` は **3 以上**（VPC の AZ 数と一致）にする。EKS Managed Nodegroup は複数 AZ の subnet を渡されると AZ にバランスよく Node を配置するため、desired<3 だと 3 AZ のうち一部 AZ に system Node が立たない。Grafana など EBS PVC を持つ Pod が「Node がいない AZ」で PV を作ると、EBS は AZ をまたげない制約から、その PV の node affinity を満たす Node が存在せず `FailedScheduling`（`didn't match PersistentVolume's node affinity`）が発生する。

`system_min_size` は env 別（dev=2 で停止/縮退時のコスト節約優先、stg/prd=3 で常時 3 AZ 配置を保証）、`system_max_size = system_desired_size + 1`（既存流儀、ローリング更新時に新 Node を立てる余裕）。

### Kafka に流す ProtoBuf は Envelope に集約する（型 1 = topic 1 = テーブル 1 を撤回）
Kafka に流れる ProtoBuf message は常に `Envelope` 1 種類とする。Envelope は `Event` を共通フィールドとして持ち、`oneof extra { OrderEvent; UserEvent; ... }` で追加情報を 1 つだけ含む（[kafka/proto/event.proto](kafka/proto/event.proto)）。理由: (1) 「Event ともう 1 つの何か」をペアで送りたい要件に proto で素直に対応できる、(2) 新しい extra 型を追加するたびに topic / Iceberg テーブル / consumer SchemaConfig を都度増やすのは運用負荷が高い、(3) consumer 側は `events.desc` から oneof case 名を動的列挙して `extra_type` 列に書き込むため、`Envelope.extra` に case を追加しても consumer コード変更が不要（[kafka/consumer/consumer.py](kafka/consumer/consumer.py) の `discover_oneof_cases`）。トレードオフ: 1 件の record で必ず Event がペイロード化されるためオーバーヘッドが微増する。Iceberg テーブル `sample_events_event` は `event_id / event_datetime / extra_type / rawdata` の 4 列で、Envelope の bytes を rawdata にそのまま保存して後段で再 deserialize する設計（=「raw envelope を保管する」）にする（[cdk/ekscdk/s3tables_stack.py](cdk/ekscdk/s3tables_stack.py)）。partition は `identity(extra_type) → day(event_datetime)` で、低カーディナリティの extra_type を先頭に置いて等値 prune を効かせる。
