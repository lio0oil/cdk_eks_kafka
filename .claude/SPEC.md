...
### Prometheus は 1 replica / retention 15 日 / gp3 PVC 20Gi
`prometheus.prometheusSpec` で `replicas: 1`、`retention: 15d`、`storageSpec.volumeClaimTemplate` に `storageClassName: gp3` の PVC 20Gi を割り当てる。1 replica は HA を捨ててコストを優先する判断（dev / コスト重視）で、Prometheus Pod が一時的に落ちても数分メトリクスが欠けるのみとする。retention 15 日は Prometheus 公式デフォルトに揃え、SLO 月次レポートが要れば段階的に伸ばす（PVC 容量は容量見積りに連動して引き上げる）。容量試算: 60s scrape × 約 70K series × 15 日 = 圧縮後 ~8GB。20Gi は約 2.5 倍のヘッドルームで、broker 増設や JMX 追加で series が伸びても対応できる余地。`replicas` を 2 にすると HA だが PVC が 2 つになり EBS 代と運用が重くなる。Alertmanager は通知先要件が出るまで `enabled: false` のまま。`scrape_interval` は chart デフォルトの 30s（PodMonitor 個別に上書き可）。
