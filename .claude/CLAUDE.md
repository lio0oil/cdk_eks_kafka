...
- **pytest**: 全テストが通過し、**カバレッジが `fail_under = 80`%（Line + Branch）以上**でないとコミット不成立。設定は `pyproject.toml` の `[tool.coverage]`。
...
- 環境変数・シークレットをコード／コミットに混入させない（`.env` は `.gitignore`、テンプレートは `.env.example`）。**取得は `app.infrastructure.settings.Settings` 経由で型安全に**（`os.environ` を直接 import しない）。
