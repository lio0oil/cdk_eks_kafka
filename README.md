...
└────────────────────────────────────────────────────────────────────────┘
                                                      │ ログ
                                                      ▼
   in-cluster Prometheus (1 replica, retention=15d) → Grafana       CloudWatch Logs
   (StatefulSet + EBS gp3 PVC 20Gi)                  (port-forward)
```
...
| メトリクス | in-cluster Prometheus（kube-prometheus-stack chart 84.5.0 同梱、1 replica / retention 15d / gp3 PVC 20Gi）/ scrape は Prometheus Operator + PodMonitor / ServiceMonitor |
...
| kube-prometheus-stack: Prometheus（StatefulSet）| system | 同上。EBS PVC は AZ をまたげないため AZ にピン留めされる |
...
#   prometheus-*-prometheus-0                (kube-prometheus-stack の Prometheus StatefulSet)
...
scrape は **in-cluster Prometheus**（kube-prometheus-stack 同梱、1 replica / retention 15d）が **Prometheus Operator + PodMonitor / ServiceMonitor** 経由で実施し、StatefulSet の EBS gp3 PVC（20Gi）に保存する。Grafana は同 chart 同梱で in-cluster Prometheus を直接 query する。
...
self-host 構成のコストは **EBS PVC（gp3）の月額**でほぼ決まる（StatefulSet pod は既存 system-nodegroup の余剰に乗る）。20Gi で約 $1.6/月程度。スケール軸:
...
4. **HA 化**: `replicas: 2` にすると PVC が 2 倍になる（メトリクス保管も二重化）。dev はコスト優先で 1 replica
