...
│  │    kube-prometheus-stack (Grafana / kube-state-metrics /     │    │
│  │      node-exporter)                                          │    │
...
└────────────────────────────────────────────────────────────────────────┘
         │ メトリクス                                  │ ログ
         │ AMP Managed Scraper が agentless で         │
         │ Pod / Node を scrape して AMP に格納        │
         ▼                                            ▼
   AMP ──▶ Grafana (in-cluster, AMP datasource を     CloudWatch Logs
            SigV4 で query / port-forward でアクセス)
```
...
| メトリクス | AMP Managed Scraper（agentless、scrape_interval=60s、設定は `manifests/amp/scrape-config.yaml` に集約） / 長期保存・query は AMP Workspace |
| 可視化 | self-hosted Grafana（kube-prometheus-stack chart 84.5.0 同梱、Prometheus / Operator / Alertmanager は無効化）/ datasource は AMP のみ（SigV4 query）/ kubectl port-forward でアクセス |
...
| AMP Managed Scraper | （EKS 外、AWS マネージド）| agentless。AWS が cross-account ENI を private subnet に挿入し、API server を介して Pod / Node を discovery |
...
- 監視環境（AMP Workspace + AMP Managed Scraper + kube-prometheus-stack[Grafana / kube-state-metrics / node-exporter のみ有効] + Fluent Bit + CloudWatch Log Group）
...
| 監視コンポーネント（AMP Workspace / AMP Managed Scraper / kube-prometheus-stack[Grafana 同梱] / Fluent Bit） | CDK（EksCdkStack） | AMP エンドポイント等の動的値（CloudFormation トークン）を注入するため |
...
#### 5-4. Grafana から AMP を query する仕組み

- **datasource は AMP のみ**（`isDefault: true`）。in-cluster Prometheus は無効化しているため Grafana 側に登録されない。chart デフォルトの in-cluster Prometheus datasource は `defaultDatasourceEnabled: false` で抑止する（残ると "Only one datasource per organization can be marked as default" で Grafana 起動失敗）
- datasource 名は **`Prometheus`** を維持（`type: prometheus`）。Strimzi 公式 dashboard を含む多くの既存 dashboard が `Prometheus` という名前の datasource をハードコード参照するため。AMP は Prometheus 互換 API なので type=prometheus で繋ぐのが正攻法
- 認証は **SigV4**。Grafana の `auth.sigv4_auth_enabled: true` を明示有効化しないと `sigV4Auth: true` 指定済みでも署名が走らず AMP から `Missing Authentication Token` (403) が返る
- IAM は EKS Pod Identity 経由で `grafana` ServiceAccount に紐付けた IAM Role（`AmazonPrometheusQueryAccess`）を使う
...
# 以下が Running であること（in-cluster Prometheus / Operator / Alertmanager は無効化済みなので立たない）:
#   *-grafana-*                              (kube-prometheus-stack の Grafana)
#   *-kube-state-metrics-*
...
# AMP Managed Scraper は AWS マネージドで EKS の外で動くため kubectl では見えない。AWS CLI で確認:
aws amp list-scrapers --query 'scrapers[?source.eksConfiguration.clusterArn!=null].[scraperId,status.statusCode]' --output table
# ACTIVE であれば scrape 中
```
...
#          IAM Role / Lambda / AMP / CloudWatch Log Group など
# 所要時間: 15〜25 分
...
scrape は **AMP Managed Scraper（agentless）** が `manifests/amp/scrape-config.yaml` の `scrape_configs` に従って実施し、AMP Workspace に格納する。Grafana は AMP を SigV4 で query する（in-cluster Prometheus は持たない）。

| 収集元 | メトリクス内容 | scrape job | 送信先 |
|---|---|---|---|
| Kafka broker / controller / Cruise Control / kafka-exporter（Strimzi）| JMX（kafka_*）/ JVM / Consumer Lag / Topic Offset | `kafka-resources-metrics`（kafka NS、`strimzi.io/kind=Kafka` の `tcp-prometheus` ポート）| AMP |
| Strimzi Cluster Operator | Reconciliation 件数・エラー率 | `cluster-operator-metrics`（strimzi-system NS）| AMP |
| Strimzi Entity Operator（Topic / User Operator）| Reconciliation 健全性 | `entity-operator-metrics`（kafka NS）| AMP |
| node-exporter（DaemonSet）| Node OS（CPU / Mem / Disk / Net）| `node-exporter`（Service 経由）| AMP |
| kube-state-metrics | K8s リソース状態（Pod / Node / PV 等）| `kube-state-metrics`（Service 経由）| AMP |
| kubelet / cAdvisor | コンテナリソース使用率・Volume stats | `kubelet` / `cadvisor`（API server proxy 経由）| AMP |
| kube-apiserver | API server SLI（latency / availability / error）| `apiserver`（default NS の `kubernetes` Service）| AMP |
| CoreDNS（kube-system）| DNS query rate / cache hit / panic | `coredns`（pod 直接 scrape の port 9153）| AMP |
| kube-proxy（kube-system、EKS Managed addon）| iptables sync latency 等 | `kube-proxy`（pod 直接 scrape の port 10249）| AMP |
| Grafana 自身（self-monitoring）| HTTP request rate / duration | `grafana`（monitoring NS）| AMP |
| Fluent Bit | コンテナログ | （AMP 対象外）| CloudWatch Logs (`/aws/eks/<cluster_name>/application`) |
| EC2 Basic monitoring | EC2 ハードウェア状態（CPU / Network / Status）| （AMP 対象外）| CloudWatch Metrics（自動、追加設定不要）|

> **CloudWatch Container Insights は採用していない**（data-on-eks 方針に揃え、メトリクスは Prometheus エコシステムに集約）。詳細は data-on-eks リファレンス参照。

### コスト感とチューニング軸

AMP の課金は **「scraper（collector hours + 収集 samples）」** と **「ingestion（取り込んだ samples 数）」** に分かれ、**ingestion が支配的**。サンプル数 = active series × scrape 回数 で効くため、削減したいときは以下の順で試す:

1. **`scrape_interval` を伸ばす**（最大効果、設定 1 行）。本プロジェクトは **60s** が既定。30s が AMP Scraper の下限、これより短くは設定不可
2. **dev クラスタを業務時間外に停止**（cluster ごと scale down or `cdk destroy`）
3. **cardinality の高いメトリクスを `metric_relabel_configs` で drop**（特に cAdvisor は container × cgroup × メトリクスで series が掛け算的に増える）
4. **不要な job を切る**（dev で apiserver 詳細を見ないなら丸ごと外す等）
...
└── constructs/
    ...
    └── monitoring.py     # AMP Workspace / AMP Managed Scraper / kube-prometheus-stack
                          # (Grafana 同梱) / Fluent Bit / Pod Identity
...
├── amp/
│   ├── scrape-config.yaml       # AMP Managed Scraper の scrape_configs（PodMonitor / ServiceMonitor から変換）
│   └── recording-rules.yaml     # AMP Rule Manager 登録用（kube-prometheus-stack 同梱の recording rule をそのまま抽出）
├── kafka/
...
└── monitoring/
    ...
    ├── kube-prometheus-stack-values.yaml       # Helm values（Grafana のみ有効、Prometheus / Operator / Alertmanager は無効化）
