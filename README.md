...
└────────────────────────────────────────────────────────────────────────┘
         ...
         ▼                                            ▼
   AMP ◀── Grafana (in-cluster)              CloudWatch Logs
         (port-forward でアクセス)
```
...
| 可視化 | self-hosted Grafana（kube-prometheus-stack 同梱）/ data source = AMP（SigV4 query）/ kubectl port-forward でアクセス |
...
| kube-prometheus-stack: Prometheus | system | 同上 |
...
- 監視環境（kube-prometheus-stack[Grafana 同梱] + Fluent Bit + AMP + CloudWatch Log Group）
...
| 監視コンポーネント（kube-prometheus-stack[Grafana 同梱] / Fluent Bit / AMP） | CDK（EksCdkStack） | AMP エンドポイント等の動的値（CloudFormation トークン）を注入するため |
...
- **default datasource**: kube-prometheus-stack 同梱の in-cluster Prometheus（最近の short-term データ）
- **AMP datasource**（追加）: 長期保存・HA 化されたメトリクス。`SigV4Auth: true` で IAM 認証
- 認証は EKS Pod Identity 経由で `grafana` ServiceAccount に紐付いた IAM Role（`AmazonPrometheusQueryAccess`）を使う。Helm values の `additionalDataSources` に `sigV4Auth: true` を指定するだけで Grafana が自動で SigV4 署名を付ける
...
└── constructs/
    ...
    └── monitoring.py     # AMP / kube-prometheus-stack (Grafana 同梱) / Fluent Bit / Pod Identity
