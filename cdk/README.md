...
│  │                                                              │    │
│  │  kafka-broker-nodegroup     (dev: t4g.large  × 3)            │    │
│  │    Kafka broker × 3 (memory-optimized)                       │    │
│  │  kafka-controller-nodegroup (dev: t4g.medium × 3)            │    │
│  │    KRaft controller × 3 (軽量・コスト最適化)                 │    │
│  │    （nodegroup 分離 + nodeAffinity で broker/controller 隔離）│    │
...
└────────────────────────────────────────────────────────────────────────┘
                                                      │ ログ
                                                      ▼
   in-cluster Prometheus (2 replica HA, retention=15d) → Grafana    CloudWatch Logs
   (StatefulSet + EBS gp3 PVC 20Gi × 2)               (port-forward)
```
...
| メトリクス | in-cluster Prometheus（kube-prometheus-stack chart 84.5.0 同梱、2 replica HA / retention 15d / gp3 PVC 20Gi × 2、AZ 跨ぎ topologySpread）/ scrape は Prometheus Operator + PodMonitor / ServiceMonitor |
...
| nodegroup | label | taint |
|---|---|---|
| `system-nodegroup` | `role=system` | **無し** |
| `kafka-broker-nodegroup` | `role=kafka-broker` | `DedicatedKafka=true:NoSchedule` |
| `kafka-controller-nodegroup` | `role=kafka-controller` | `DedicatedKafka=true:NoSchedule` |

### Pod の配置先

| Pod | nodegroup | 配置の根拠 |
|---|---|---|
| Strimzi Operator | system | toleration 無し → kafka taint で弾かれ、system のみ |
| AWS Load Balancer Controller | system | 同上 |
| kube-prometheus-stack: Prometheus（StatefulSet × 2 replica）| system | 同上。AZ 跨ぎ topologySpreadConstraints で別 AZ に促す。EBS PVC は AZ をまたげないため各 replica は AZ にピン留めされる |
| kube-prometheus-stack: Prometheus Operator | system | 同上 |
| kube-prometheus-stack: Grafana | system | 同上 |
...
| node-exporter / fluent-bit (DaemonSet) | 全ノード | DaemonSet | 被災 AZ のノード分のメトリクス・ログが欠落。他 AZ 分は継続 | 同左（範囲が拡大） |
| NLB / Endpoint Service | AWS マネージド | 3 AZ | AWS 側で他 AZ にルーティング切替 | 残 1 AZ の broker への接続継続 |
...
- 監視環境（kube-prometheus-stack[Prometheus / Operator / Grafana / kube-state-metrics / node-exporter] + Strimzi 系 PodMonitor + Fluent Bit + CloudWatch Log Group）
...
| 監視コンポーネント（kube-prometheus-stack / Strimzi 系 PodMonitor / Fluent Bit） | CDK（EksCdkStack） | log group ARN 等の動的値（CloudFormation トークン）を注入するため。PodMonitor は Prometheus Operator (kps) と対象 NS の作成順序を CDK で担保 |
...
| Grafana ダッシュボード | CDK（ConfigMap として apply、Grafana sidecar が自動取り込み） | `manifests/monitoring/dashboards/` の ConfigMap に `grafana_dashboard: "1"` ラベル。追加・更新は YAML 編集 → `cdk deploy` |
...
### 5. Grafana（self-hosted）アクセスとダッシュボード

可視化レイヤは kube-prometheus-stack 同梱の Grafana を使う（AMG ではない）。**ConfigMap を CDK で apply するだけで sidecar が自動取り込み**するため、AMG 時代のような手動インポート手順は不要。

#### 5-1. ブラウザからアクセス

Grafana Service は ClusterIP のみで外部公開しない。`kubectl port-forward` で接続する。

```bash
# Service 名を確認（kube-prometheus-stack の release 名 + "-grafana"）
kubectl get svc -n monitoring | grep grafana

# port-forward（別タブで起動しっぱなしにする）
kubectl port-forward svc/<release>-grafana -n monitoring 3000:80

# ブラウザで http://localhost:3000 を開く
# 初期認証: admin / admin（Helm values の adminPassword）
```

> **本番運用での注意**: `manifests/monitoring/kube-prometheus-stack-values.yaml` の `adminPassword` は開発用に平文 `admin`。本番では Secrets Manager や `existingSecret` 参照に置き換える。

#### 5-2. ダッシュボードはすべて自動取り込み
...
これらは `manifests/monitoring/dashboards/` に ConfigMap 形式で配置されており、`monitoring.py` が `cluster.add_manifest()` で apply する。Grafana sidecar が `grafana_dashboard: "1"` ラベルを検知して**自動取り込み**するため、`cdk deploy` 後に Grafana を開けば既に配置されている。

ダッシュボードを追加・更新する場合:

1. `manifests/monitoring/dashboards/` に新しい ConfigMap YAML を追加（`labels: { grafana_dashboard: "1" }` を付ける）
2. `monitoring.py` の dashboard ファイル一覧に追記
3. `cdk deploy EksCdkStack`

#### 5-3. Community dashboard の取り込み

Node OS / Cluster / Pod レベルの汎用 dashboard は `grafana.com` で公開されている。ConfigMap として Git 管理に取り込む手順:

| dashboard ID | 名前 | カバー範囲 |
|---|---|---|
| **1860** | Node Exporter Full | Node OS 全部入り（CPU / Memory / Disk / Network） |
| 15757 | Kubernetes / Compute Resources / Cluster | クラスタ全体俯瞰 |
| 15760 | Kubernetes / Compute Resources / Node (Pods) | Node 上の Pod 別配分 |
| 15758 | Kubernetes / Compute Resources / Namespace (Pods) | Namespace 内の Pod 別 |
| 15761 | Kubernetes / Compute Resources / Pod | Pod 単体の詳細 |

```bash
# 例: Node Exporter Full (ID 1860) を ConfigMap 化
ID=1860
NAME=node-exporter-full
REV=$(curl -sS "https://grafana.com/api/dashboards/${ID}" | python3 -c "import sys,json;print(json.load(sys.stdin)['revision'])")
JSON=$(curl -sS "https://grafana.com/api/dashboards/${ID}/revisions/${REV}/download")

cat > manifests/monitoring/dashboards/grafana-${NAME}-dashboard.yaml <<EOF
apiVersion: v1
kind: ConfigMap
metadata:
  name: ${NAME}-dashboard
...
その後 `monitoring.py` のダッシュボード一覧に追記して `cdk deploy`。

#### 5-4. Grafana から in-cluster Prometheus を query する仕組み

- **datasource は in-cluster Prometheus のみ**（chart デフォルトで自動登録、`name: Prometheus`、`isDefault: true`、URL は `http://<release>-kube-prometheus-stack-prometheus:9090`）。AMP は使わない
- datasource 名は **`Prometheus`** を維持。Strimzi 公式 dashboard を含む多くの既存 dashboard が `Prometheus` という名前の datasource をハードコード参照するため
- 認証は不要（cluster 内 Service との通信のみ）。Grafana の Pod Identity SA は付与しているが managed policy は何も attach していない
...
# 以下が Running であること（Alertmanager は無効化済みなので立たない）:
#   prometheus-*-prometheus-0 / prometheus-*-prometheus-1  (Prometheus StatefulSet 2 replica HA)
#   *-prometheus-operator-*                  (Operator が PodMonitor / ServiceMonitor を翻訳)
#   *-grafana-*
#   *-kube-state-metrics-*
...
# Prometheus が PodMonitor / ServiceMonitor を読めているか確認:
kubectl get podmonitors,servicemonitors -A
# Strimzi 系 3 件 (kafka-resources-metrics / cluster-operator-metrics /
# entity-operator-metrics) と chart 同梱 ServiceMonitor 群が見えるはず。

# Prometheus の active targets を確認（port-forward 経由）:
kubectl port-forward -n monitoring svc/<release>-kube-prometheus-stack-prometheus 9090:9090
# ブラウザで http://localhost:9090/targets を開き、State=UP を確認
```
...
```bash
for ng in system-nodegroup kafka-broker-nodegroup kafka-controller-nodegroup; do
...
#          IAM Role / Lambda / Prometheus 用 EBS PVC / CloudWatch Log Group など
# 所要時間: 15〜25 分
...
scrape は **in-cluster Prometheus**（kube-prometheus-stack 同梱、2 replica HA / retention 15d、AZ 跨ぎ topologySpread）が **Prometheus Operator + PodMonitor / ServiceMonitor** 経由で実施し、StatefulSet の EBS gp3 PVC（各 replica 20Gi、合計 40Gi）に独立保存する。Grafana は同 chart 同梱で Service 経由（2 Pod 間でロードバランス）で in-cluster Prometheus を直接 query する。

| 収集元 | メトリクス内容 | scrape 元 |
|---|---|---|
| Kafka broker / controller / Cruise Control / kafka-exporter（Strimzi）| JMX（kafka_*）/ JVM / Consumer Lag / Topic Offset | PodMonitor `kafka-resources-metrics`（kafka NS、`strimzi.io/kind=Kafka` の `tcp-prometheus` ポート）|
| Strimzi Cluster Operator | Reconciliation 件数・エラー率 | PodMonitor `cluster-operator-metrics`（strimzi-system NS）|
| Strimzi Entity Operator（Topic / User Operator）| Reconciliation 健全性 | PodMonitor `entity-operator-metrics`（kafka NS）|
| node-exporter（DaemonSet）| Node OS（CPU / Mem / Disk / Net）| ServiceMonitor（chart 同梱）|
| kube-state-metrics | K8s リソース状態（Pod / Node / PV 等）| ServiceMonitor（chart 同梱）|
| kubelet / cAdvisor | コンテナリソース使用率・Volume stats | ServiceMonitor（chart 同梱）|
| kube-apiserver | API server SLI（latency / availability / error）| ServiceMonitor（chart 同梱、`default/kubernetes` 経由）|
| CoreDNS / kube-proxy | DNS query 率 / iptables sync 等 | ServiceMonitor（chart 同梱、kube-system NS）|
| Grafana / Prometheus / Operator 自身 | self-monitoring | ServiceMonitor（chart 同梱）|
| Fluent Bit | コンテナログ | （メトリクスではなくログ。CloudWatch Logs `/aws/eks/<cluster_name>/application`）|
| EC2 Basic monitoring | EC2 ハードウェア状態（CPU / Network / Status）| CloudWatch Metrics（AWS 自動、追加設定不要）|

> **CloudWatch Container Insights は採用していない**（data-on-eks 方針に揃え、メトリクスは Prometheus エコシステムに集約）。詳細は data-on-eks リファレンス参照。

### コスト・容量・retention のチューニング軸

self-host 構成のコストは **EBS PVC（gp3）の月額**でほぼ決まる（StatefulSet pod は既存 system-nodegroup の余剰に乗る）。20Gi × 2 replica = 40Gi で約 $3.2/月程度。スケール軸:

1. **active series 数**: cardinality の高い container メトリクス（cAdvisor 等）が支配的。不要なら chart values の `kubelet.serviceMonitor.cAdvisorRelabelings` 等で drop
2. **retention**: 長くするほど PVC 使用量が線形に増える。`prometheus.prometheusSpec.retention` を変えたら `storageSpec.volumeClaimTemplate.spec.resources.requests.storage` も連動で見直す
3. **`scrape_interval`**: chart デフォルトの 30s。伸ばすと PVC 使用量も連動して減るが、SLO ダッシュボードの精度が下がるトレードオフ
4. **HA / replicas**: `replicas: 2`（現状）で PVC が 2 倍。`replicas: 1` にすればコスト半減だが AZ・Pod 単一障害でメトリクスが完全に欠ける期間が出る
...
└── constructs/
    ...
    └── monitoring.py     # CloudWatch Log Group / kube-prometheus-stack
                          # (Prometheus / Operator / Grafana 同梱) / Strimzi 系
                          # PodMonitor 3 件 / Fluent Bit / Pod Identity
...
│   ├── kafka-pod-monitor.yaml             # Strimzi Kafka resources PodMonitor (broker / controller / cc / exporter)
│   ├── cluster-operator-pod-monitor.yaml  # Strimzi Cluster Operator PodMonitor (strimzi-system NS)
│   ├── entity-operator-pod-monitor.yaml   # Strimzi Entity Operator PodMonitor (Topic / User Operator)
│   └── target-group-binding.yaml # AWS LBC 用 TargetGroupBinding（テンプレート、bootstrap+broker N 適用）
└── monitoring/
    ├── namespace.yaml                          # monitoring namespace
    ├── kube-prometheus-stack-values.yaml       # Helm values（Prometheus 1 replica / retention 15d / gp3 PVC 20Gi、Alertmanager 無効）
    ├── fluent-bit-values.yaml                  # Helm values（CloudWatch Logs 出力）
    └── dashboards/                              # Grafana ダッシュボード ConfigMap（CDK が apply、sidecar が自動取り込み）
```
